...
func (tunnel *SSHtunnel) Close() {
	...
	tunnel.wg.Wait()
	tunnel.log.Debugf("tunnel %s closed", tunnel.String())
}

func (tunnel *SSHtunnel) Start() error {
	var err error
	tunnel.log.Debugf("dialing ssh: %v", tunnel.String())
	...
	for key, t := range tunnel.tunnels {
		tunnel.log.Debugf("starting listener %s -> %s", t.Local.String(), t.Remote.String())
